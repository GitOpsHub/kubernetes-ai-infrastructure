...
here it's a guarded wrapper around `terraform destroy` (see §7), only relevant if you went beyond the
lab and ran a real `apply`.
...
- Read [chapter 00](../00-prerequisites-and-cluster-setup/README.md) first — this chapter's module
  is a Terraform rewrite of exactly what chapter 00's `eks/create-cluster.sh` + `eks/cluster.yaml` do.
  Read those scripts alongside the `.tf` files here to see the CLI-to-Terraform mapping directly.
...
| Time | Activity |
|---|---|
| 0:00–0:20 | Read §3 (concepts), compare the `.tf` module here to chapter 00's matching shell script |
...
module rather than writing `aws_eks_cluster` + `aws_eks_node_group` by hand.
...
bucket or fail, and this chapter's validation is deliberately backend-free).
...
an S3 bucket, so this is safe to run with zero cloud credentials.
...
deliberately doesn't create for you, to keep the lab account-free).
...
| `terraform init` fails to download the `vpc` or `eks` submodule | Registry unreachable, or a `~>` pin that no longer resolves (module got yanked or majors moved on) | Check `registry.terraform.io/modules/terraform-aws-modules/eks/aws` for the current latest major before bumping the pin |
