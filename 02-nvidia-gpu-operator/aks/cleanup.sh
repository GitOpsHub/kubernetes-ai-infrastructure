#!/usr/bin/env bash
# Uninstall the GPU Operator release. Does not touch the AKS GPU node pool (see
# 01-gpu-nodes-and-scheduling/aks/scale-gpu-pool.sh MIN=0 or cleanup.sh for that).
set -euo pipefail
