...
echo "Cluster-side objects removed. The S3 bucket (\$VELERO_S3_BUCKET) and its backup contents"
echo "are NOT deleted by this script -- backups should outlive the cluster that made them."
echo "Delete it explicitly once you no longer need any of these backups:"
echo "  source ./bucket.env && aws s3 rb s3://\${VELERO_S3_BUCKET} --force"
