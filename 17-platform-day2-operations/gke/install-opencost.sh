...
# existing kube-prometheus-stack (must already be installed: 04-gpu-observability/gke).
set -euo pipefail
...
if ! kubectl -n monitoring get svc kube-prometheus-stack-prometheus >/dev/null 2>&1; then
  ...
  echo "kube-prometheus-stack-prometheus failed) -- run 04-gpu-observability/gke first." >&2
  exit 1
fi
...
helm upgrade --install opencost opencost/opencost \
  ...
  -f "${HERE}/values-opencost-gke.yaml" \
  --wait --timeout 10m
